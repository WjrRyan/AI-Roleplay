  ...
  const prompt = `
    ...
    2. **核心能力维度评估** (每个维度最高5分，最低1分。基础分4分，有禁忌行为扣1分，表现卓越加1分):
       - **维度一：反馈有效性 (SBI模型应用)**
         ...
         - 禁忌行为审查 (forbiddenBehaviors): 严格检查是否出现以下红线行为。若存在，记录具体行为描述：
           1. 模糊主观表述 (如"我觉得你态度不行"、"你不够努力"而无事实支撑)。
           2. 推卸责任给上级 (如"是老板让我给你C的"、"我也没办法")。
       
       - **维度二：教练式引导能力 (GROW模型应用)**
         - 检查是否有效提问引导，而非直接给答案。
         - 禁忌行为审查 (forbiddenBehaviors): 严格检查是否出现以下红线行为。若存在，记录具体行为描述：
           1. 只谈问题忽视改进 (只批评不给建议，对话结束时没有下一步行动)。
           2. 面谈准备缺失 (对员工的具体业务细节一问三不知，泛泛而谈)。

       - **维度三：积极倾听与同理心**
         - 检查是否回应情绪、不打断、复述确认。
         - 禁忌行为审查 (forbiddenBehaviors): 严格检查是否出现以下红线行为。若存在，记录具体行为描述：
           1. 频繁打断员工发言 (员工话没说完就被打断)。
           2. 忽视情绪 (员工表达委屈时，经理完全无视，机械地继续讲道理)。
           3. 攻击性语言 (使用侮辱、讽刺或威胁性词汇)。
    
    3. **特殊干预检测**:
       - 检测经理是否使用了“轮流坐庄”逻辑 (rotationFallacyDetected)。即：“因为名额有限，这次轮到你了”或“为了平衡团队，今年必须有人拿C”这种错误说法。此为严重管理事故，必须准确识别。
